gameRootPanel = nil
gameMapPanel = nil
local gameRightPanels = nil
local gameLeftPanels = nil
local gameBottomPanel = nil
local gameBottomActionPanel = nil
local gameLeftActionPanel = nil
local gameRightActionPanel = nil
local gameLeftActions = nil
local gameTopBar = nil
local logoutButton = nil
local mouseGrabberWidget = nil
local countWindow = nil
local logoutWindow = nil
local exitWindow = nil
local bottomSplitter = nil
local hookedMenuOptions = {}
local selectedThing, selectedType, selectedSubtype
...
function save()
  if not bottomSplitter then return end
  local settings = {}
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if not classic and not g_app.isMobile() then shortcut = '(Shift)' end
  if lookThing then
    menu:addOption(tr('Look'), function() g_game.look(lookThing) end, shortcut)
  end

  if not classic and not g_app.isMobile() then shortcut = '(Ctrl)' end
  if useThing then
  ...
  if creatureThing then
    ...
    if creatureThing:isLocalPlayer() then
      ...
      if not classic and not g_app.isMobile() then shortcut = '(Alt)' end
      ...
      if creatureThing:isPlayer() then
        ...
        if modules.game_console and modules.game_console.isIgnored(creatureName) then
...
function moveStackableItem(item, toPos)
  ...
  countWindow.onDestroy = function()
    countWindow = nil
  end
  ...
  local oldOnDestroy = countWindow.onDestroy
  countWindow.onDestroy = function()
    for _, handle in ipairs(keybinds) do
      if handle then
        g_keyboard.unbindKeyPress(handle)
      end
    end
...
function getContainerPanel()
  ...
  containerPanel = math.max(1, containerPanel)
...
function refreshViewMode()  
  ...
  for i=1,rightCount+leftCount do
    local panel
    if i > rightCount then
      panel = gameLeftPanels:getChildByIndex(i - rightCount)
    else
...
function setupLeftActions()
  ...
  if gameLeftActions.attack then
    gameLeftActions.attack.doubleClickAction = function()
      local battlePanel = modules.game_battle and modules.game_battle.battlePanel
