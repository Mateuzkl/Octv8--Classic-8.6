...
local overlay = nil
local healthCircleFront = nil
local manaCircleFront = nil
local healthCircle = nil
local manaCircle = nil
local topHealthBar = nil
local topManaBar = nil
local healthInfoButton = nil
...
local function loadIcon(bitChanged, content)
...
local function offline()
  if not healthInfoWindow then return end
  local conditionPanel = healthInfoWindow:recursiveGetChildById('conditionPanel')
...
local function onHealthChange(localPlayer, health, maxHealth)
  ...
  local healthPercent = math.floor(localPlayer:getHealthPercent())
...
local function onManaChange(localPlayer, mana, maxMana)
  ...
  local manaPc = maxMana > 0 and math.floor(mana * 100 / maxMana) or 0
  local Ymppc = math.floor(208 * (1 - (manaPc / 100)))
  local rect = { x = 0, y = Ymppc, width = 63, height = 208 - Ymppc + 1 }
  manaCircleFront:setImageClip(rect)
  manaCircleFront:setImageRect(rect)
end

local function onLevelChange(localPlayer, value, percent)
...
local function hideExperience()
  if not experienceBar then return end
  local removeHeight = experienceBar:getMarginRect().height
  experienceBar:setVisible(false)
  healthInfoWindow:setHeight(math.max(healthInfoWindow.minimizedHeight, healthInfoWindow:getHeight() - removeHeight))
end

local function onOverlayGeometryChange() 
  ...
  if classic then
    ...
    local parentMarginTop = parent and parent:getMarginTop() or 0
    topHealthBar:setMarginTop(45 - parentMarginTop)
    topManaBar:setMarginTop(45 - parentMarginTop)
    minMargin = 200
  end
