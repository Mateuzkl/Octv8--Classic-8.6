...
function EnterGame.focusAccount()
  local account = enterGame:getChildById('accountNameTextEdit')
  ...
  -- focus chain manually (same pattern as the chat fix in game_console)
  local focusChain = function()
    if account:isDestroyed() then return end
    account:focus()
    local w = account:getParent()
    while w do
      w:focus()
      w = w:getParent()
    end
  ...
  focusChain()
  focusEvent = addEvent(focusChain)
end
...
function EnterGame.doLogin(account, password, token, host)
  ...
  G.clientVersion = clientVersionSelector and tonumber(clientVersionSelector:getText())
  ...
  if modules.game_things and modules.game_things.isLoaded() then
...
function EnterGame.onLoginError(err)
  ...
  local errorBox = displayErrorBox(tr('Login Error'), err)
  errorBox.onOk = EnterGame.show
  if err and (err:lower():find("invalid") or err:lower():find("not correct") or err:lower():find("or password")) then
    EnterGame.clearAccountFields()
  end
end
