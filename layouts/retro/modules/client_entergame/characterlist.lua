...
local ensureVisibleEvent
local scheduleAutoReconnect
local executeAutoReconnect
local lastLogout = 0
...
local function onLoginWait(message, time)
  ...
  if updateWaitEvent then removeEvent(updateWaitEvent); updateWaitEvent = nil end
  updateWaitEvent = scheduleEvent(function() updateWait(g_clock.seconds(), g_clock.seconds() + time) end, 0)
  if resendWaitEvent then removeEvent(resendWaitEvent); resendWaitEvent = nil end
...
local function onGameLoginToken(unknown)
  CharacterList.destroyLoadBox()
...
local function onGameConnectionError(message, code)
  ...
  if (not g_game.isOnline() or code ~= 2) and not errorBox then -- code 2 is normal disconnect, end of file
    ...
    local isConnecting = protocolGame and protocolGame:isConnecting()
    local text = translateNetworkError(code, isConnecting, message)
...
local function onGameUpdateNeeded(signature)
  CharacterList.destroyLoadBox()
...
function CharacterList.terminate()
 if USE_NEW_ENERGAME then return end
  ...
  if ensureVisibleEvent then
    removeEvent(ensureVisibleEvent)
    ensureVisibleEvent = nil
  end
...
function CharacterList.create(characters, account, otui)
  ...
  if focusLabel then
    ...
    if ensureVisibleEvent then removeEvent(ensureVisibleEvent); ensureVisibleEvent = nil end
    ensureVisibleEvent = addEvent(function() characterList:ensureChildVisible(focusLabel) end)
  end
...
function CharacterList.show()
  ...
  if autoReconnectButton then autoReconnectButton:setOn(autoReconnect) end
end
