...
function init()
  g_settings.remove('autoSwitchPreset')
  ...
  setupEvent = addEvent(function() setup() end)
...
function terminate()
  ...
  if optionsButton then optionsButton:destroy(); optionsButton = nil end
  if audioButton then audioButton:destroy(); audioButton = nil end
  extrasPanel = nil
  generalPanel = nil
  interfacePanel = nil
  consolePanel = nil
...
function setOption(key, value, force)
  ...
  elseif key == 'showFps' then
    ...
    if modules.game_stats and modules.game_stats.ui and modules.game_stats.ui.fps then
  ...
  elseif key == 'topHealthManaBar' then
    if modules.game_healthinfo then
  ...
  elseif key == "chatMode" then
    ...
    if modules.game_console then
      ...
      if modules.game_console.consoleToggleChat and modules.game_console.consoleToggleChat:isChecked() ~= check then
        modules.game_console.consoleToggleChat:setChecked(check)
      end
      if value == CHAT_MODE.ON and not modules.game_console.isChatEnabled() then
        modules.game_console.enableChat()
      elseif value == CHAT_MODE.OFF and modules.game_console.isChatEnabled() then
...
function showHotkeys()
  if modules.game_hotkeys and modules.game_hotkeys.show then
...
function presetOption(widget, key, value)
  setOption(key, value)
end
