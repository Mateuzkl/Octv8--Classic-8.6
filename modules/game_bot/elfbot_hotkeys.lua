...
local function msg(text)
  ...
  g_logger.info("[ElfBot Hotkeys] " .. text)
end
...
local function evalCondition(cond)
  ...
  if not fn then
    g_logger.warning("[ElfBot Hotkeys] Condicao invalida: " .. cond .. " | " .. tostring(err))
    return false
  end
...
local function executeOneCommand(cmd)
  ...
  g_logger.warning("[ElfBot Hotkeys] Comando ainda nao suportado: " .. cmd)
end
...
function elfHotkeysInit()
  ...
  elfHotkeysWindow = g_ui.displayUI('elfhotkeys')
  elfHotkeysWindow:hide()
  if elfHotkeysWindow.closeButton then
    elfHotkeysWindow.closeButton.onClick = function()
      elfHotkeysWindow:hide()
    end
  end
...
-- Export functions to modules.game_bot so OTUI onclick and sandbox can find them
