...
local function getDefaultHtmlParent()
...
function Controller:loadHtml(path, parent)
    ...
    parent = parent or getDefaultHtmlParent()
    self:setUI(path, parent)
    self.htmlId = g_html.load(self.name, path, parent)
    if not self.htmlId or self.htmlId == 0 then
        self.htmlId = nil
        self.ui = nil
        return
    end
...
function Controller:clearScheduledEvents()
    for _, events in pairs(self.scheduledEvents or {}) do
        if events ~= nil then
...
function Controller:centerHtmlWindow(window)
    window = window or self.ui
    if not window or window:isDestroyed() then
        return
    end

    local parent = window:getParent()
    local desiredParent = self.dataUI and self.dataUI.parent or getDefaultHtmlParent()

    if desiredParent and (not parent or parent:isDestroyed()) then
        window:setParent(desiredParent)
    ...
    if window.updateParentLayout then
...
function Controller:scheduleHtmlCenter()
    if not self.ui or self.ui:isDestroyed() then
        return
    end

    self:centerHtmlWindow()

    for _, delay in ipairs({ 1, 10, 50 }) do
        self:scheduleEvent(function()
            self:centerHtmlWindow()
        end, delay, '__html_center_' .. delay)
    end
end
